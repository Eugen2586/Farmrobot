...
// Idee: int primSteps = 100; // Laenge eines Steps / Rechteckwellenanzahl (waere denkbar)
 ...
 const int LENGTHUNIT = 10; //Defines a standard length

 void doStuff(){
  if(digitalRead(DIR_Y) != 0){
    position++;  
  }else{
    position--;
  }
...
void DIR_ZD(int steps) {
 ...
 void initMotors(){ //Motorsteuerung initialisieren, dabei Pinbelegung am Chip definieren
  pinMode(ENABLE_Y, OUTPUT); //ENABLE_X
  pinMode(DIR_Y, OUTPUT); //DIR_Xection
  pinMode(PULS_Y, OUTPUT); //PULS_Xe
